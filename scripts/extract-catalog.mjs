...
5b. 【極重要】若訂購碼第①欄是「規格代號/型式」且列出多個系列變體 (例如 MA/MAC/MSA/MTA/MAD/MAJ/MAR 代表復動/緩衝可調/單動押出/單動引入/雙軸/行程可調等不同型式)，你必須把「全部」變體代號當成第一個 category (id 用 "series") 的 options 完整列出，format 開頭放 {series}，且該系列的 code 設為空字串 ""。絕對不可以只保留一個代號 (如只留 "MA") 而把其他變體丟掉——那會遺失單動/雙軸/可調等不同產品。同一個「規格代號」欄位裡的所有代號都要收進 series options。
